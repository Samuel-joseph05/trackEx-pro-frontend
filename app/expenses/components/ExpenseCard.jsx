...
import { Calendar, Clock, IndianRupee, Tag } from "lucide-react";

import DeleteExpenseDialog from "./DeleteExpenseDialog";

const ExpenseCard = ({ expense, handleEdit, handleDelete }) => {
  return (
    <Card className="group overflow-hidden rounded-3xl border border-slate-700 bg-white/10 backdrop-blur-xl transition-all duration-300 hover:-translate-y-2 hover:border-indigo-500 hover:shadow-[0_20px_60px_rgba(79,70,229,0.35)]">
      <CardContent className="p-6">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-2xl font-bold text-white">{expense.title}</h2>

            <p className="mt-1 text-sm text-slate-400">Expense Record</p>
          </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4">
          <DeleteExpenseDialog expense={expense} handleDelete={handleDelete} />
